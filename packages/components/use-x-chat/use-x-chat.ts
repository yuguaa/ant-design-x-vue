...
>(config: XChatConfig<AgentMessage, ParsedMessage, Input, Output>) {
  ...
  const getTransformMessage: TransformMessageFn<AgentMessage, Output> = (params) => {
    ...
    // Compatible bug fixes, onSuccess(output) => onSuccess(output[]),before v1.2.8
  ...
  const onRequest = useEventCallback((requestParams: RequestParams<AgentMessage> | Input | SimpleType) => {
    if (!agent)
      throw new Error(
